...
/*   Updated: 2022/11/18 21:09:12 by vimercie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	philo_init(t_data *data, int i)
{
	...
	data->p[i].thinking = malloc(sizeof(int));
	if (data->p[i].philo_id == NULL)
	{
		free(data->p[i].philo_id);
		return (0);
	}
	if (data->p[i].thinking == NULL)
	{
		free(data->p[i].thinking);
		return (0);
	}
...
int	data_init(t_data *data)
{
	...
	if (data->threads == NULL || data->p == NULL
		|| data->forks_id == NULL || data->stop == NULL)
	{
		free(data->threads);
		free(data->forks_id);
		free(data->p);
